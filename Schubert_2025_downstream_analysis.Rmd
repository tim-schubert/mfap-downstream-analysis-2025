...
# Robust min-max scaling for the severity side annotation
severity_span <- diff(range(df_heat$severity_score, na.rm = TRUE))
df_heat <- df_heat %>%
  mutate(
    severity_01 = if_else(
      severity_span == 0,
      0.5,
      (severity_score - min(severity_score, na.rm = TRUE)) / severity_span
    )
  )
...
symptom_cols_all <- setdiff(
  names(df_heat),
  c("patient_ID", "severity_score", "severity_01")
)
...
png(
  filename = file.path(heatmap_out_dir, "target_heatmap_by_severity_complexheatmap.png"),
  width = 18,
  height = 12,
  units = "in",
  res = 1200,
...
svg(
  filename = file.path(heatmap_out_dir, "target_heatmap_by_severity_complexheatmap.svg"),
  width = 18,
  height = 12,
  bg = "white"
)
